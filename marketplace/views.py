...
#chat test
from .models import Conversation, Message, Notification, Feedback, Assignment, Payment, Interview
...
def interview_list(request):
    if request.user.role == "client":  # recruiter
    ...
    return render(
        request,
        "marketplace/interview_list.html",
        {"interviews": interviews}
    )


@login_required
def interview_create(request, proposal_id):
    proposal = get_object_or_404(Proposal, id=proposal_id)
    job = proposal.job

    # Only recruiter can schedule
    if request.user != job.client:
        messages.error(request, "You are not allowed to schedule this interview.")
    ...
    if request.method == "POST":
        ...
        Notification.objects.create(
            ...
            text=f"🗓️ Interview scheduled for {job.title} on {interview_date} at {interview_time}"
        )
        ...
        if proposal.freelancer.email:
            send_mail(
                ...
                message=(
                    f"Hi {proposal.freelancer.first_name or 'Freelancer'},\n\n"
                    f"Good news! 🎉 Your interview has been scheduled.\n\n"
                    f"📌 Job Title: {job.title}\n"
                    f"👤 Recruiter: {request.user.first_name or 'Recruiter'}\n"
                    f"🗓 Date: {interview_date}\n"
                    f"⏰ Time: {interview_time}\n"
                    f"💻 Mode: {mode.capitalize()}\n"
                    f"{f'🔗 Meeting Link: {meeting_link}\n' if meeting_link else ''}\n"
                    "Please be available on time and make sure you have a stable internet connection if it’s an online interview.\n\n"
                    "You can view interview details anytime from your dashboard.\n\n"
                    "Best wishes,\n"
                    "Team SkillConnect\n"
                    "Connecting Talent with Opportunity 🚀"
                ),
    ...
    return render(
        request,
        "marketplace/interview_create.html",
        {"proposal": proposal}
    )
...
@login_required
def interview_mark_completed(request, interview_id):
    interview = get_object_or_404(Interview, id=interview_id)

    # Only recruiter can mark completed
    ...
    # 🔔 Notification to freelancer (optional)
    Notification.objects.create(
        user=interview.freelancer,
        text=f"✅ Interview completed for {interview.job.title}"
    )

    messages.success(request, "Interview marked as completed ✅")
    return redirect("interview_list")  # update if your page name differs
...
def dashboard_stats(request):
    ...
    if role == "client":
        ...
        proposals_qs = Proposal.objects.filter(job__client=user)

        # ✅ Earnings analytics (Recruiter Paid)
        total_paid_amount = Payment.objects.filter(recruiter=user, status="paid").aggregate(
            total=Sum("amount")
        )["total"] or 0

        success_rate = round((proposals_qs.filter(status="accepted").count() / max(1, proposals_qs.count())) * 100, 1)

        # ✅ Weekly jobs created
        jobs_week = jobs_qs.filter(created_at__date__gte=today - timedelta(days=6))
        jobs_by_day = jobs_week.annotate(day=TruncDate("created_at")).values("day").annotate(count=Count("id"))

        jobs_count_map = {x["day"]: x["count"] for x in jobs_by_day}
        jobs_weekly_counts = [jobs_count_map.get(d, 0) for d in last_7_days]

        # ✅ Weekly proposals created
        props_week = proposals_qs.filter(created_at__date__gte=today - timedelta(days=6))
        props_by_day = props_week.annotate(day=TruncDate("created_at")).values("day").annotate(count=Count("id"))

        props_count_map = {x["day"]: x["count"] for x in props_by_day}
        props_weekly_counts = [props_count_map.get(d, 0) for d in last_7_days]    
        ...
        context = {
            ...
            "inprogress_jobs": jobs_qs.filter(status="in_progress").count(),
            "completed_jobs": jobs_qs.filter(status="completed").count(),

            # PROPOSAL STATS
            "total_proposals": proposals_qs.count(),
            "pending_proposals": proposals_qs.filter(status="pending").count(),
            "accepted_proposals": proposals_qs.filter(status="accepted").count(),
            "rejected_proposals": proposals_qs.filter(status="rejected").count(),

            # OTHER
            "total_assignments": Assignment.objects.filter(recruiter=user).count(),
            "submitted_assignments": Assignment.objects.filter(recruiter=user, status="submitted").count(),
            "total_interviews": Interview.objects.filter(recruiter=user).count(),
            "payments_done": Payment.objects.filter(recruiter=user, status="paid").count(),
            "chats": Conversation.objects.filter(recruiter=user).count(),

            # ✅ NEW FOR CHARTS
            "total_paid_amount": float(total_paid_amount),

            "weekly_labels": [d.strftime("%d %b") for d in last_7_days],
            "jobs_weekly_counts": jobs_weekly_counts,
            "proposals_weekly_counts": props_weekly_counts,
            "success_rate": success_rate,
        }

    # ✅ Freelancer Statistics
    elif role == "freelancer":
        proposals_qs = Proposal.objects.filter(freelancer=user)

        # ✅ Earnings analytics (Freelancer Received)
        total_received_amount = Payment.objects.filter(freelancer=user, status="paid").aggregate(
            total=Sum("amount")
        )["total"] or 0
        ...
        context = {
            ...
            "active_contracts": proposals_qs.filter(status="accepted", job__status="in_progress").count(),
            "total_assignments": Assignment.objects.filter(freelancer=user).count(),
            "submitted_assignments": Assignment.objects.filter(freelancer=user, status="submitted").count(),
            "total_interviews": Interview.objects.filter(freelancer=user).count(),
            "payments_received": Payment.objects.filter(freelancer=user, status="paid").count(),
            "chats": Conversation.objects.filter(freelancer=user).count(),
    ...
    return render(request, "marketplace/dashboard_stats.html", context)
